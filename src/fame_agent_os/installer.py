...
3. Call `fame_preflight`, then `fame_prepare_task` before modifying files. If a worktree path is returned, work there only.
4. Delegate to the exact selected Fame custom agent. Do not independently choose Sol/Terra/Luna or duplicate its work.
5. Call `fame_finish_task` after implementation so deterministic verification and state recording run before reporting completion.
