...
from .config import merged_config, project_config, project_root
...
TOOLS = {
    ...
    "fame_status": {}, "fame_verify": {}, "fame_usage": {"task_id": "string?"},
    "fame_prepare_task": {"task": "string", "budget": "string?", "max_tier": "string?", "human_approved": "boolean?"},
...
def doctor(root: Path) -> dict:
    ...
    return {"fame_version": __import__("fame_agent_os").__version__, "project_initialized": fame_dir(root).is_dir(),
            ...
            "git": git, "production": {"enabled": production, "deterministic_verification_configured": bool(commands),
            "live_checkout_clean": _git_clean(root), "safe": not production or bool(commands)},
            ...
            "safety_problems": (["production verification commands are not configured"] if production and not commands else []) + (["production live checkout is not clean"] if production and commands and not _git_clean(root) else [])}
...
def preflight_result(root: Path, args: dict) -> dict:
    ...
    if production and not config.get("verification", {}).get("commands", []): problems.append("production verification commands are not configured")
...
def finish(root: Path, args: dict) -> dict:
    ...
    config = project_config(workspace); result = verify(workspace, config.get("verification", {}).get("commands", []))
    if args.get("success") is False: result = type(result)(False, [{"error": "extension reported failure"}] + result.results)
    task = json.loads(task_path.read_text()); task["verification"] = {"success": result.success, "results": result.results};
    ...
    return {"success": result.success, "task_id": task_id, "status": task["status"], "workspace": str(workspace),
            ...
            "verification": task["verification"], "automatic_merge": False, "automatic_deploy": False}
...
def call(root: Path, name: str, args: dict) -> dict:
    ...
    if name == "fame_verify":
        result = verify(root, project_config(root).get("verification", {}).get("commands", [])); return {"success": result.success, "results": result.results}
...
def serve(root: Path | None = None) -> int:
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            elif method == "tools/list": result = {"tools": [{"name": name, "description": f"Deterministic Fame operation: {name}", "inputSchema": {"type": "object", "properties": {k: {"type": "boolean" if v == "boolean?" else "string"} for k, v in schema.items()}}} for name, schema in TOOLS.items()]}
